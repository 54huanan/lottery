...
function initAll() {
  window.AJAX({
    ...
    success(data) {
      ...
      let curLucks = currentPrize ? basicData.luckyUsers[currentPrize.type] : 0;
...
function bindEvent() {
  document.querySelector("#menu").addEventListener("click", function(e) {
    ...
    switch (target) {
      ...
      case "lottery":
        let pickedNum = 0;
        for (let key of Object.keys(basicData.luckyUsers)) {
          pickedNum += basicData.luckyUsers[key].length;
        }
        // 所有奖品抽完，不能再抽奖
        if (pickedNum === 22) {
          addQipao("很遗憾，所有奖品已抽完");
          return;
        }
...
function changeCard(cardIndex, user) {
  ...
  card.innerHTML = `<div class="company">${COMPANY}</div><div class="name">${user[1]}</div>`;
  /* <div class="details">${user[0]}<br/>${user[2] || "PSST"}</div> */
}
  ...
  window.AJAX({
    ...
    success(data) {
      if (data.type === "success") {
        downloadFile(data.url, "中奖名单.xlsx");
      }
    }
  });
}
    ...
    document.addEventListener(
      ...
      function(event) {
        ...
        if (keyCode == 13) {
          ...
          if (lotteryBtn.offsetWidth > 0) lotteryBtn.click();
        }
      },
      false
    );
